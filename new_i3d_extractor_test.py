...
if __name__ == '__main__':
    old_feat = np.load('C:/Users/jplabuser/Downloads/UCF_Test_ten_i3d/UCF_Test_ten_i3d/Normal_Videos_006_x264_i3d.npy')
    new_feat = np.load('../../I3D_Feature_Extraction_resnet/output/samplevideos/Normal_Videos_006_x264.npy')
    summarize_feat(old_feat, "old")
    summarize_feat(new_feat, "new")

    compare_feat_cosine(old_feat, new_feat)
    model = Model_V2(n_features = 2048).to(device)
    ckpt = torch.load("../../C2FPL/ckpt/UCFfinal(git).pkl", map_location=device)
